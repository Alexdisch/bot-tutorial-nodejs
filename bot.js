...
function respond() {
  var request = JSON.parse(this.req.chunks[0]),
      ...
      botRegexP = /^\/PDL/i;  botRegexTw = /^\/twitch/i; botRegexZink = /^\/zink/; botRegexSh = /^\/shrug/; botRegexWk = /^\/users/; botRegexCC = /^\/cc/;
  ...
  if(request.text && botRegex.test(request.text)) {
    ...
    this.res.writeHead(200);
    postMessage("http://daddyleagues.com/MHL/players?name=&position=all&team="+request.text.substring(5,8));
    //postMessage("http://daddyleagues.com/MHL/team/"+request.text.substring(5,8)+"/depthchart");
    this.res.end();
  } 
  
  else if(request.text && botRegexOW.test(request.text)) {
    this.res.writeHead(200);
    postMessage("www.daddyleagues.com/MHL/maddenrating/");
    this.res.end();
  } 
    ...
    this.res.writeHead(200);
    postMessage("https://www.daddyleagues.com/MHL/rules");
    this.res.end();
  } 
  else if(request.text && botRegexTrades.test(request.text)) {
    this.res.writeHead(200);
    postMessage("https://www.daddyleagues.com/MHL/forum");
    this.res.end();
  } 
  else if(request.text && botRegexSC.test(request.text)) {
    this.res.writeHead(200);
    
    postMessage("http://daddyleagues.com/MHL/team/"+request.text.substring(5,8)+"/schedule");
    this.res.end();
  }
    ...
    var rep = req.replace(/ /,"+");
    postMessage("http://daddyleagues.com/MHL/players?name="+rep+"&position=all&team=all");
  ...
  else if(request.text && botRegexZink.test(request.text)) {
    this.res.writeHead(200);
    postMessage("https://media3.giphy.com/media/3o84sv2u7KSHKbwPza/giphy.gif?cid=3640f6095c23b1b34a5773555117540a");
    this.res.end();
  } 
